...
| `[text](url)` | `<a href>` ★未決 |
| `[text](#名前)` | **ページへの参照**。`#` だけならホーム、`#dev` は `contents/dev.txt` |
| `![alt](src)` | `<img>` |
| `` `code` `` | `<code>` |
| `**text**` | `<strong>` |
| 裸の行 | テキスト。連続すると `<p>` |
...
行き先を綴らずにページを指す。`.html` の付き方や出力名を書く側が知らなくて済む。
...
- **無いページを指したらエラー。** 綴りの間違いも消したページも、そこで止まる
- **いま出しているページを指していたら `aria-current="page"` を付ける。**
  骨格は全ページに焼き込まれるので `.txt` 側からは現在地を書きようがない。
  出している最中のページを知っているのはパーサだけ
- 行頭の `#`（見出し）とは位置が違うので衝突しない
